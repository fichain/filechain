...
var rpcHandlers = map[string]struct {
	...
	"getfilehashinfo": {handlerWithChain: listSingleHash},
	"getbuyhashinfo":  {handlerWithChain: getBuyHashInfo},
	"appealtransfee":  {handlerWithChain: appealTransFee},
}
...
func getAddressesByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	account, err := w.AccountNumber(waddrmgr.KeyScopeBIP0084, cmd.Account)
	if err != nil {
		return nil, err
	}
...
func getBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	if accountName == "*" {
		...
		var account uint32
		account, err = w.AccountNumber(waddrmgr.KeyScopeBIP0084, accountName)
		if err != nil {
			return nil, err
		}
		...
		freezed = bals.Freezed
	}
	return &btcjson.GetBalanceResult{
		Balance: balance.ToBTC(),
		Freezed: freezed.ToBTC(),
	}, nil
}
...
func getAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	acctName, err := w.AccountName(waddrmgr.KeyScopeBIP0084, account)
	if err != nil {
...
func getAccountAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	addr, err := w.CurrentAddress(account, waddrmgr.KeyScopeBIP0084)
	if err != nil {
		return nil, err
	}

	passhelper, err := w.GetPassHelper()
	if err != nil {
		return nil, err
	}
	...
	return &struct {
		Addr   string
		Helper string
	}{Addr: addr.EncodeAddress(), Helper: passhelper}, err
}
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	account, err := w.AccountNumber(waddrmgr.KeyScopeBIP0084, acctName)
	if err != nil {
		return nil, err
	}
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	_, err = w.ImportPrivateKey(waddrmgr.KeyScopeBIP0084, wif, nil, *cmd.Rescan)
	switch {
...
func createNewAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	_, err := w.NextAccount(waddrmgr.KeyScopeBIP0084, cmd.Account)
	if waddrmgr.IsError(err, waddrmgr.ErrLocked) {
		return nil, &btcjson.RPCError{
...
func renameAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	account, err := w.AccountNumber(waddrmgr.KeyScopeBIP0084, cmd.OldAccount)
	if err != nil {
		return nil, err
	}
	return nil, w.RenameAccount(waddrmgr.KeyScopeBIP0084, account, cmd.NewAccount)
}
...
func getNewAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	addr, err := w.NewAddress(account, waddrmgr.KeyScopeBIP0084)
	if err != nil {
		return nil, err
	}
...
func getRawChangeAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	addr, err := w.NewChangeAddress(account, waddrmgr.KeyScopeBIP0084)
	if err != nil {
		return nil, err
	}
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	results, err := w.TotalReceivedForAccounts(
		waddrmgr.KeyScopeBIP0084, int32(*cmd.MinConf),
	)
	if err != nil {
		return nil, err
	}
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, cred := range details.Credits {
		...
		_, addrs, _, _, err := txscript.ExtractPkScriptAddrs(
		...
		if err == nil && len(addrs) == 1 {
			...
			account, err := w.AccountOfAddress(addr)
			if err == nil {
				name, err := w.AccountName(waddrmgr.KeyScopeBIP0084, account)
				if err == nil {
					accountName = name
				}
			}
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	results, err := w.AccountBalances(waddrmgr.KeyScopeBIP0084, int32(*cmd.MinConf))
	if err != nil {
		return nil, err
	}
...
func sendPairs(w *wallet.Wallet, amounts map[string]btcutil.Amount,
	...
	tx, err := w.SendOutputs(outputs, &keyScope, account, minconf, feeSatPerKb, "", nil, btcutil.Amount(0), nil)
	if err != nil {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	account, err := w.AccountNumber(
		waddrmgr.KeyScopeBIP0084, cmd.FromAccount,
	)
	if err != nil {
		return nil, err
	}
	...
	return sendPairs(w, pairs, waddrmgr.KeyScopeBIP0084, account, minConf,
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	account, err := w.AccountNumber(waddrmgr.KeyScopeBIP0084, cmd.FromAccount)
	if err != nil {
		return nil, err
	}
	...
	return sendPairs(w, pairs, waddrmgr.KeyScopeBIP0084, account, minConf, txrules.DefaultRelayFeePerKb)
}
...
func validateAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	acctName, err := w.AccountName(
		waddrmgr.KeyScopeBIP0084, ainfo.InternalAccount(),
	)
	if err != nil {
		return nil, &ErrAccountNameNotFound
	}
	...
	case waddrmgr.ManagedScriptAddress:
		...
		class, addrs, reqSigs, _, err := txscript.ExtractPkScriptAddrs(
...
// 用户发起传输费申诉，希望举证某个传输费
func appealTransFee(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*btcjson.AppealTransferFeeCmd)
	proof, err := decodeHexStr(cmd.Proof)
	if err != nil {
		return nil, err
	}
	...
	transfer, err := btcutil.DecodeAddress(string(info.Transfer), w.ChainParams())
	if err != nil {
		return nil, err
	}

	buyinfo, err := chainClient.GetBuyHashInfo(&btcjson.GetBuyHashCmd{
		Hash: hex.EncodeToString(info.BuyHash),
	})
	if err != nil {
		return nil, err
	}

	return sendAppealTransFee(w, buyinfo, transfer, proof, waddrmgr.KeyScopeBIP0084, waddrmgr.DefaultAccountNum, 1,
		txrules.DefaultRelayFeePerKb)
}
...
// 需要创建多个txout
// 如果成功
//    需要发送给作者购买的费用（ TODO 需考虑金额为 0 的情况）
//    需要发送给多个传输者传输的费用
// 如果失败
//    需要发送给传输者传输的费用
func confirmBuy(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*btcjson.ConfirmBuyCmd)
	buyhash, err := decodeHexStr(cmd.Hash)
	if err != nil {
		return nil, err
	}
	var total int64
	for _, fee := range cmd.Fees {
		if fee.Fee <= 0 {
			return nil, fmt.Errorf("确认传输费必须是大于0")
		}
		afee, err := btcutil.NewAmount(fee.Fee)
		if err != nil {
			return nil, err
		}
		total += int64(afee)
	...
	feetotal, err := btcutil.NewAmount(buyinfo.TransFee)
	if err != nil {
		return nil, err
	}
	fileinfo, err := chainClient.ListSingleHash(&btcjson.ListSingleHashCmd{
		Hash: buyinfo.FileHash,
	})
	if err != nil {
		return nil, err
	}

	filehash, err := hex.DecodeString(buyinfo.FileHash)
	if err != nil {
		return nil, err
	}

	if total > int64(feetotal) {
		return nil, fmt.Errorf("总确认传输费不能大于总计划费用")
	}

	author, err := btcutil.DecodeAddress(fileinfo.Author, w.ChainParams())
	if err != nil {
		return nil, err
	}
	switch author.(type) {
	case *btcutil.AddressWitnessPubKeyHash:
		break
	default:
		return nil, fmt.Errorf("不支持的地址格式")
	}
	price, err := btcutil.NewAmount(fileinfo.Price)
	if err != nil {
		return nil, err
	}

	outputs := make([]*wire.TxOut, 0, len(cmd.Fees)+1)
	if total > int64(feetotal)>>1 {
		//超过50%算作传输成功
		toSellerScript, err := txscript.ConfirmBuyFeeScript(buyhash, filehash, author)
		if err != nil {
			return nil, fmt.Errorf("cannot create txout script: %s", err)
		}
		outputs = append(outputs, wire.NewTxOut(int64(price), toSellerScript))
	}
	for _, fee := range cmd.Fees {
		transfer, err := btcutil.DecodeAddress(fee.Account, w.ChainParams())
		if err != nil {
			return nil, err
		}
		afee, err := btcutil.NewAmount(fee.Fee)
		...
		toTransferScript, err := txscript.ConfirmHashTransferFeeScript(buyhash, transfer)
		if err != nil {
			return nil, fmt.Errorf("cannot create txout script: %s", err)
		}
		outputs = append(outputs, wire.NewTxOut(int64(afee), toTransferScript))
	}

	hash, err := hex.DecodeString(buyinfo.TransFeeTx)
	if err != nil {
		return nil, err
	}

	// 指定购买时候花费的押金作为本次交易唯一的输入
	...
	txin := wire.TxIn{
		PreviousOutPoint: wire.OutPoint{
			Hash:  *chash,
			Index: buyinfo.TransFeeVoutIdx,
		},
		Sequence: wire.MaxTxInSequenceNum - 1,
	}

	buyaddr, err := btcutil.DecodeAddress(string(buyinfo.Buyer), w.ChainParams())
	if err != nil {
		return nil, err
	}
	script, err := txscript.BuyHashFeeScript(filehash, buyaddr, buyhash)
	if err != nil {
		return nil, err
	}
	tx, err := w.SendOutputs(outputs, &waddrmgr.KeyScopeBIP0084, waddrmgr.DefaultAccountNum, 1,
		txrules.DefaultRelayFeePerKb, "", &txin, btcutil.Amount(int64(feetotal)<<2+int64(price)<<1), script)
	if err != nil {
		if err == txrules.ErrAmountNegative {
			return "", ErrNeedPositiveAmount
		}
...
// 购买一个hash，实际的传输费押金是宣称的4倍，将在购买确认100个块之后归还
...
func buyHash(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	transfee, err := btcutil.NewAmount(cmd.TransFee)
	if err != nil {
		return nil, err
	}
	if transfee < 0 {
	...
	addresses, err := w.AccountAddresses(waddrmgr.DefaultAccountNum)
	if err != nil {
		return nil, err
	}
	if len(addresses) == 0 {
		return nil, &btcjson.RPCError{
			...
			Message: "wallet has no default address",
		}
	}
	myaddr := addresses[0]

	info, err := chainClient.ListSingleHash(&btcjson.ListSingleHashCmd{
		Hash: cmd.Hash,
	})
	if err != nil {
		return nil, err
	...
	price, err := btcutil.NewAmount(info.Price)
	if err != nil {
		return nil, err
	}

	seeds := sha256.Sum256(append(hash, myaddr.ScriptAddress()...))
	seeds1 := seeds[:]
	buyhash := sha1.Sum(seeds1)

	// 购买总押金的两倍的文件价格 + 4倍的承诺传输费
	// 在确认成功购买的时候，将单倍的文件价格和最多1倍的传输费转给相应的收款方，剩余金额以押金形成保留100个块以等待是否有人申诉
	// 如果购买失败，只需要传输相应的传输费给收款方，但是如果传输费作弊被投诉，仍将损失全部剩余押金
	outputs := make([]*wire.TxOut, 0, 1)
	toSellerScript, err := txscript.BuyHashFeeScript(hash, myaddr, buyhash[:])
	if err != nil {
		return nil, fmt.Errorf("cannot create txout script: %s", err)
	}
	outputs = append(outputs, wire.NewTxOut(int64(price)<<1+int64(transfee)<<2, toSellerScript))

	tx, err := w.SendOutputs(outputs, &waddrmgr.KeyScopeBIP0084, waddrmgr.DefaultAccountNum, 1,
		txrules.DefaultRelayFeePerKb, "", nil, btcutil.Amount(0), nil)
	if err != nil {
		if err == txrules.ErrAmountNegative {
			return "", ErrNeedPositiveAmount
		}
...
// 购买者在传输过程中可以给传输者发送传输证明，如果将来购买者在传输确认中作弊的话
// 传输者可以通过这个proof 从传输押金中获取到自己的传输费
func getProof(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*btcjson.GetProofCmd)
	buyhash, err := decodeHexStr(cmd.BuyHash)
	if err != nil {
		return nil, err
	}
	...
	_, err = decodeAddress(cmd.Transfer, w.ChainParams())
	if err != nil {
		return nil, err
	}
...
// 发布一个文件hash
func publishHash(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*btcjson.PublishHashCmd)

	amt, err := btcutil.NewAmount(cmd.Price)
	if err != nil {
		return nil, err
	}
	...
	author := cmd.AuthorAddr
	if author == nil {
		a := addresses[0].EncodeAddress()
		author = &a
	}

	var filetag uint8
	if cmd.IsOriginal != nil && *cmd.IsOriginal {
		filetag |= 1 << btcjson.Original
	}
	if cmd.IsPolitically != nil && *cmd.IsPolitically {
		filetag |= 1 << btcjson.Politically
	}
	if cmd.IsSexually != nil && *cmd.IsSexually {
		filetag |= 1 << btcjson.Sexually
	}
	if cmd.IsViolence != nil && *cmd.IsViolence {
		filetag |= 1 << btcjson.Violence
	}

	//发布hash的押金等于价格的10倍或者是规定的默认最小押金
	atLeastTxOut := amt * txrules.DefaultDepositTimesOfPrice
	if atLeastTxOut < txrules.DefaultMinimalDepositAmountForPublishFile {
		atLeastTxOut = txrules.DefaultMinimalDepositAmountForPublishFile
	}
	...
	if len(hash) != 20 {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCTxRejected,
			Message: "hash string should be 20 bytes len",
		}
	}
	...
	if len(cmd.Title) <= 5 {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCTxRejected,
			Message: "title should be at least 5 length",
		}
	}

	return sendPublishHash(w, btcjson.PublicHash{
		TxOut:       atLeastTxOut,
		Title:       []byte(cmd.Title),
		Price:       amt,
		Hash:        hash,
		FileTag:     filetag,
		Author:      []byte(*author),
		DepositAddr: addresses[0],
	}, waddrmgr.KeyScopeBIP0084, waddrmgr.DefaultAccountNum, 1,
		txrules.DefaultRelayFeePerKb)
}

// 申诉传输费，将直接引用押金 Vout as Vin, 计算自己本来应该得到的传输费作为 Vout
// 剩余金额传输到特定社区基金地址
func sendAppealTransFee(w *wallet.Wallet, buyinfo *btcjson.BuyHashInfoRsp, addr btcutil.Address, proof []byte,
	keyScope waddrmgr.KeyScope, account uint32, minconf int32,
	feeSatPerKb btcutil.Amount) (string, error) {

	transfee, err := btcutil.NewAmount(buyinfo.TransFee)
	if err != nil {
		return "", err
	}

	depositLeft, err := btcutil.NewAmount(buyinfo.DepositValueLeft)
	if err != nil {
		return "", err
	}

	payToMe, err := txscript.PayToAddrScript(addr)
	if err != nil {
		return "", fmt.Errorf("cannot create txout script: %s", err)
	}

	prevhash, err := hex.DecodeString(buyinfo.TransFeeTx)
	if err != nil {
		return "", err
	}
	hash, err := chainhash.NewHash(prevhash)
	if err != nil {
		return "", err
	}
	...
	proof, err = txscript.ProofScript(proof)
	if err != nil {
		return "", err
	}
	witness := make([][]byte, 0)
	witness = append(witness, proof)
	txin.Witness = witness
	txout1 := wire.TxOut{
		Value:    int64(transfee),
		PkScript: payToMe,
	}
	buyhash, err := hex.DecodeString(buyinfo.BuyHash)
	if err != nil {
		return "", err
	}
	filehash, err := hex.DecodeString(buyinfo.FileHash)
	...
	script, err := txscript.BuyHashTransDepositScript(filehash, buyhash)
	if err != nil {
		return "", err
	}
	tx, err := w.SendOutputs([]*wire.TxOut{
		&txout1,
	}, &waddrmgr.KeyScopeBIP0084, waddrmgr.DefaultAccountNum, 1,
		txrules.DefaultRelayFeePerKb, "", &txin, btcutil.Amount(depositLeft), script)
	if err != nil {
		if err == txrules.ErrAmountNegative {
			return "", ErrNeedPositiveAmount
		}
...
func sendPublishHash(w *wallet.Wallet, publichash btcjson.PublicHash,
	keyScope waddrmgr.KeyScope, account uint32, minconf int32,
	feeSatPerKb btcutil.Amount) (string, error) {

	outputs, err := makePublishHashOutputs(publichash, w.ChainParams())
	if err != nil {
		return "", err
	}
	tx, err := w.SendOutputs(outputs, &keyScope, account, minconf, feeSatPerKb, "", nil, btcutil.Amount(0), nil)
...
func makePublishHashOutputs(info btcjson.PublicHash, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, 1)
	publishScript, err := txscript.PublishScript(info.Hash, info.Price, info.Title, info.Author, info.FileTag, info.DepositAddr)
	if err != nil {
		return nil, fmt.Errorf("cannot create txout script: %s", err)
	}
	outputs = append(outputs, wire.NewTxOut(int64(info.TxOut), publishScript))
	return outputs, nil
}

func getBuyHashInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*btcjson.GetBuyHashCmd)
	return chainClient.GetBuyHashInfo(cmd)
}

// listSingleHash 通过向主链全节点发送请求，获取单个文件hash的信息
func listSingleHash(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*btcjson.ListSingleHashCmd)
	return chainClient.ListSingleHash(cmd)
}

// listHash 通过向主链全节点发送请求，获取文件hash的信息
func listHash(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*btcjson.ListHashCmd)
