...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, keyScope *waddrmgr.KeyScope,
	account uint32, minconf int32, feeSatPerKb btcutil.Amount, dryRun bool, txIn *wire.TxIn, totalIn btcutil.Amount, script []byte) (
	...
	addrmgrNs, cs, err := w.addrMgrWithChangeSource(
	...
	var changeSource = cs
	if txIn != nil {
		// 如果是调用者指定的txIn，通常都是指定消费押金的情形
		// 一种是发送购买确认事务的时候，需要将购买押金中的一部分转换成实际支付，剩余的转换成传输押金
		// 一种是申诉的时候消费传输押金，或者100个块以后消费传输押金
		inputSource = func(target btcutil.Amount) (btcutil.Amount, []*wire.TxIn,
			[]btcutil.Amount, [][]byte, error) {
			return totalIn, []*wire.TxIn{
				txIn,
			}, []btcutil.Amount{totalIn}, [][]byte{script}, nil
		}
		//对于第一种情况，我们需要定制 changeSource，添加 OP_FILE_BUY_TRANS 传输标记
		if script[len(script)-1] == txscript.OP_FILE_BUY_PAY {
			changeSource = &txauthor.ChangeSource{
				NewScript: func() ([]byte, error) {
					//将购买押金转换成传输押金的时候，不能使用支付脚本，因为这个脚本需要能够被proof解锁
					param, err := txscript.ExtractFCEOperation(script)
					if err != nil {
						return nil, err
					}
					ns, err := txscript.BuyHashTransDepositScript(param.Params[1].([]byte), param.Params[0].([]byte))
					if err != nil {
						return nil, err
					}
					return ns, nil
		...
		} else if script[len(script)-1] == txscript.OP_FILE_BUY_TRANS {
			//对于第二种情况，如果是传输费申诉，我们需要将剩余金额发送至社区地址
			if len(txIn.Witness) == 1 && txIn.Witness[0][len(txIn.Witness[0])-1] == txscript.OP_FILE_APPEAL_TRANSFEE {
				changeSource = &txauthor.ChangeSource{
					NewScript: func() ([]byte, error) {
						addr, err := btcutil.DecodeAddress(blockchain.CommunityAddr, w.chainParams)
						if err != nil {
							return nil, err
						}
						return txscript.PayToAddrScript(addr)
					},
					ScriptSize: cs.ScriptSize,
				}
			}
		}
		//其它情况下，这笔钱可以任意支配
	...
	// be valid.
	// 终于找到了罪魁祸首，原来是你小子在这里给我随机打乱的output的顺序
	if tx.ChangeIndex >= 0 {
...
func (w *Wallet) findEligibleOutputs(dbtx walletdb.ReadTx,
	...
	hashmap, err := BuildFileDepositMapFromUnspent(unspent)
	if err != nil {
		return nil, err
	}
	...
	for i := range unspent {
		...
		_, addrs, _, fcparam, err := txscript.ExtractPkScriptAddrs(
		...
		// 如果是被冻结的押金，则暂时先不花
		if fcparam != nil && !FCUnspentIsPayable(fcparam, hashmap, output.Height, syncBlock.Height) {
			continue
		}
