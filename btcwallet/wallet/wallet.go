...
var (
	...
	wmetaNamespaceKey    = []byte("wmeta")

	passwdHelperName = []byte("passwdhelper")
)
...
type (
	createTxRequest struct {
		...
		txIn        *wire.TxIn // 有时候我们想指定消费某条押金，可以fix想要消费的txin
	...
	for {
		...
		case txr := <-w.createTxRequests:
			...
			tx, err := w.txToOutputs(
				...
				txr.minconf, txr.feeSatPerKB, txr.dryRun, txr.txIn, txr.totalIn, txr.scriptIn,
			)
			heldUnlock.release()
...
func (w *Wallet) CreateSimpleTx(keyScope *waddrmgr.KeyScope, account uint32,
	...
	dryRun bool, txIn *wire.TxIn, totalIn btcutil.Amount, scriptIn []byte) (*txauthor.AuthoredTx, error) {
...
type Balances struct {
	...
	Freezed        btcutil.Amount
}

func BuildFileDepositMapFromUnspent(unspent []wtxmgr.Credit) (*map[string]struct {
	exists   bool
	confirms int
}, error) {
	...
	for i := range unspent {
		output := &unspent[i]
		params, err := txscript.ExtractFCEOperation(output.PkScript)
		if err != nil {
			return nil, err
		}
		// 文件链扩展了两种可消费押金，一种是文件发布押金，需要成功下载次数超过10次才可以解锁
		// 一种是传输费押金，需要100个块以后才可以解锁
		// 文件传输押金解锁的时候需要特殊的签名才可以
		// 一种是购买费用，要等发布押金解锁以后才可以使用
		if params != nil {
			if params.Code == txscript.OP_FILE_PUBLISH { // 发布押金
				filehash := hex.EncodeToString(params.Params[0].([]byte))
				if value, ok := result[filehash]; ok {
					value.exists = true
				} else {
					result[filehash] = struct {
						exists   bool
						confirms int
					}{
						exists:   true,
			...
			} else if params.Code == txscript.OP_FILE_CONFIRM_BUY { //确认购买
				filehash := hex.EncodeToString(params.Params[1].([]byte))
				if value, ok := result[filehash]; ok {
					value.confirms += 1
				} else {
					result[filehash] = struct {
...
func FCUnspentIsPayable(params *txscript.FileChainParams, hashmap *map[string]struct {
	exists   bool
	confirms int
}, outputHeight int32, syncHeight int32) bool {
	if params == nil {
		return true
	}
	if params.Code == txscript.OP_FILE_PUBLISH { // 发布押金
		filehash := hex.EncodeToString(params.Params[0].([]byte))
		if (*hashmap)[filehash].confirms >= 10 {
			return true
		} else {
			return false
		}
		...
		filehash := hex.EncodeToString(params.Params[1].([]byte))
		if !(*hashmap)[filehash].exists {
			return true
		} else {
			return false
		}
	} else if params.Code == txscript.OP_FILE_BUY_TRANS { //传输押金
		if outputHeight+100 < syncHeight {
			return true
		} else {
			return false
		}
...
func (w *Wallet) CalculateAccountBalances(account uint32, confirms int32) (Balances, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		depositMap, err := BuildFileDepositMapFromUnspent(unspent)
		if err != nil {
			return err
		}
		...
		for i := range unspent {
			...
			_, addrs, _, params, err := txscript.ExtractPkScriptAddrs(
			...
			if output.FromCoinBase && !confirmed(int32(w.chainParams.CoinbaseMaturity),
				...
				if params == nil || FCUnspentIsPayable(params, depositMap, output.Height, syncBlock.Height) {
					bals.Spendable += output.Amount
				} else {
					bals.Freezed += output.Amount
				}
			}
		}
		return nil
...
func (w *Wallet) GetPassHelper() (string, error) {
	var helper string
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		bucket := tx.ReadBucket(wmetaNamespaceKey)
		helper = string(bucket.Get(passwdHelperName))
		return nil
	})
	if err != nil {
		return "", err
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		var address string
		var accountName string
		_, addrs, _, fcparam, _ := txscript.ExtractPkScriptAddrs(output.PkScript, net)
		if len(addrs) == 1 {
			addr := addrs[0]
		...
		if fcparam != nil {
			log.Info("列举事务结果", send, spentCredit, isCredit, fcparam.String())
		}
		if send || spentCredit {
			if fcparam != nil && fcparam.Code == txscript.OP_FILE_PUBLISH {
				result.Category = "扣除发布文件押金:" + hex.EncodeToString(fcparam.Params[0].([]byte))
			} else {
				result.Category = "转出"
			}
		...
		if isCredit {
			...
			if fcparam != nil && fcparam.Code == txscript.OP_FILE_PUBLISH {
				result.Category = "转入发布文件押金:" + hex.EncodeToString(fcparam.Params[0].([]byte))
			} else {
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		for i := range unspent {
			output := unspent[i]
			var outputAcct uint32
			_, addrs, _, _, err := txscript.ExtractPkScriptAddrs(output.PkScript, w.chainParams)
			if err == nil && len(addrs) > 0 {
				_, outputAcct, err = w.Manager.AddrAccount(addrmgrNs, addrs[0])
			}
			if err == nil {
...
func (w *Wallet) ListUnspent(minconf, maxconf int32,
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		for i := range unspent {
			...
			sc, addrs, _, _, err := txscript.ExtractPkScriptAddrs(
				output.PkScript, w.chainParams)
			if err != nil {
				continue
			}
		...
		rangeFn := func(details []wtxmgr.TxDetails) (bool, error) {
			for i := range details {
				...
				for _, cred := range detail.Credits {
					pkScript := detail.MsgTx.TxOut[cred.Index].PkScript
					var outputAcct uint32
					_, addrs, _, _, err := txscript.ExtractPkScriptAddrs(pkScript, w.chainParams)
					if err == nil && len(addrs) > 0 {
						_, outputAcct, err = w.Manager.AddrAccount(addrmgrNs, addrs[0])
					}
					if err == nil {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		rangeFn := func(details []wtxmgr.TxDetails) (bool, error) {
			for i := range details {
				...
				for _, cred := range detail.Credits {
					pkScript := detail.MsgTx.TxOut[cred.Index].PkScript
					_, addrs, _, _, err := txscript.ExtractPkScriptAddrs(pkScript,
						w.chainParams)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, keyScope *waddrmgr.KeyScope,
	...
	label string, txIn *wire.TxIn, totalIn btcutil.Amount, scriptIn []byte) (*wire.MsgTx, error) {
	...
	createdTx, err := w.CreateSimpleTx(
		keyScope, account, outputs, minconf, satPerKb, false, txIn, totalIn, scriptIn,
	)
	if err != nil {
		return nil, err
	}
...
// of the wallet. --------- I don't think so
...
func (w *Wallet) publishTransaction(tx *wire.MsgTx) (*chainhash.Hash, error) {
	...
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L953
	...
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/rpcserver.go#L3355
	...
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L1036
	...
	// referenced outputs.
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L591
	...
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/blockchain/chain.go#L405
	...
	// allowed.
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L1409
	...
	// whatever reason.
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L841
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L854
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L875
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L896
	// https://github.com/filechain/filechain/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L913
