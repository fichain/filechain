...
func (l *Loader) GetWallet() *Wallet {
	return l.wallet
}
...
func (l *Loader) CreateNewWallet(pubPassphrase, privPassphrase, seed []byte,
	bday time.Time, passhelper string) (*Wallet, error) {

	// 增加私钥解锁密码提示
	l.OnWalletCreated(func(tx walletdb.ReadWriteTx) error {
		bucket, err := tx.CreateTopLevelBucket(wmetaNamespaceKey)
		if err != nil {
			return err
		}
		return bucket.Put(passwdHelperName, []byte(passhelper))
	})
