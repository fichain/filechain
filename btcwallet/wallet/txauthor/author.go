...
func NewUnsignedTransaction(outputs []*wire.TxOut, feeRatePerKb btcutil.Amount,
	...
	for {
		...
		if inputAmount < targetAmount+targetFee {
			fmt.Printf("余额不足，需要 %f,拥有 %f", (targetAmount + targetFee).ToBTC(), inputAmount.ToBTC())
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []btcutil.Amount,
	...
	for i := range inputs {
		if len(inputs[i].SignatureScript) > 0 || len(inputs[i].Witness) > 0 {
			//已经在别的地方签过名了(proof)，就不需要签名了
			continue
		}
