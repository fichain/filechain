...
func TestTxToOutputsDryRun(t *testing.T) {
	...
	dryRunTx, err := w.txToOutputs(txOuts, nil, 0, 1, 1000, true, nil, btcutil.Amount(0), nil)
	if err != nil {
	...
	dryRunTx2, err := w.txToOutputs(txOuts, nil, 0, 1, 1000, true, nil, btcutil.Amount(0), nil)
	if err != nil {
		t.Fatalf("unable to author tx: %v", err)
	}
	...
	// to the database.
	tx, err := w.txToOutputs(txOuts, nil, 0, 1, 1000, false, nil, btcutil.Amount(0), nil)
	if err != nil {
		t.Fatalf("unable to author tx: %v", err)
	}
