...
const (
	...
	OP_NOP9                  = 0xb8 // 184
	OP_NOP10                 = 0xb9 // 185
	OP_FILE_PUBLISH          = 0xba // 186
	OP_FILE_BUY_PAY          = 0xbb // 187
	OP_FILE_BUY_TRANS        = 0xbc // 188
	OP_FILE_CONFIRM_BUY      = 0xbd // 189
	OP_FILE_CONFIRM_TRANSFEE = 0xbe // 190
	OP_FILE_APPEAL_TRANSFEE  = 0xbf // 191
...
type FECParamType uint8

const (
	FCEP_DATA_ARRAY FECParamType = iota
	FCEP_NUMBER
)

type FCEOPStruct struct {
	ParamFormats []FECParamType // 这个OPCode后面跟了多少个参数
	BuyHashValid bool           // 参数是否包含购买hash
}

var FCEOpCodeDefine = map[uint8]*FCEOPStruct{
	OP_FILE_PUBLISH: {
		ParamFormats: []FECParamType{FCEP_DATA_ARRAY, FCEP_NUMBER, FCEP_DATA_ARRAY, FCEP_DATA_ARRAY, FCEP_NUMBER},
		BuyHashValid: false,
	},
	OP_FILE_BUY_PAY: {
		ParamFormats: []FECParamType{FCEP_DATA_ARRAY, FCEP_DATA_ARRAY},
		BuyHashValid: true,
	},
	OP_FILE_BUY_TRANS: {
		ParamFormats: []FECParamType{FCEP_DATA_ARRAY, FCEP_DATA_ARRAY},
	...
	OP_FILE_CONFIRM_TRANSFEE: {
		ParamFormats: []FECParamType{FCEP_DATA_ARRAY},
		BuyHashValid: true,
	},
	OP_FILE_APPEAL_TRANSFEE: {
		ParamFormats: []FECParamType{FCEP_DATA_ARRAY},
		BuyHashValid: false,
	},
}
...
var opcodeArray = [256]opcode{
	...
	// Undefined opcodes.
	OP_FILE_PUBLISH:          {OP_FILE_PUBLISH, "OP_FILE_PUBLISH", 1, opcode5Drop},
	OP_FILE_BUY_PAY:          {OP_FILE_BUY_PAY, "OP_FILE_BUY_PAY", 1, opcode2Drop},
	OP_FILE_BUY_TRANS:        {OP_FILE_BUY_TRANS, "OP_FILE_BUY_TRANS", 1, opcode3DropPushTrue},
	OP_FILE_CONFIRM_BUY:      {OP_FILE_CONFIRM_BUY, "OP_FILE_CONFIRM_BUY", 1, opcode2Drop},
	OP_FILE_CONFIRM_TRANSFEE: {OP_FILE_CONFIRM_TRANSFEE, "OP_FILE_CONFIRM_TRANSFEE", 1, opcodeDrop},
	OP_FILE_APPEAL_TRANSFEE:  {OP_FILE_APPEAL_TRANSFEE, "OP_FILE_APPEAL_TRANSFEE", 1, opcodeNop},
...
// opcode2Drop removes the top 2 items from the data stack.
//
// Stack transformation: [... x1 x2 x3 x4] -> [... x1]
func opcode5Drop(op *parsedOpcode, vm *Engine) error {
	return vm.dstack.DropN(5)
}

func opcode3DropPushTrue(op *parsedOpcode, vm *Engine) error {
	err := vm.dstack.DropN(2)
	if err != nil {
		return err
	}
