...
func ProofScript(proof []byte) ([]byte, error) {
	return NewScriptBuilder().AddData(proof).AddOp(OP_FILE_APPEAL_TRANSFEE).Script()
}
...
func extractScriptInt64(opcode parsedOpcode) (int64, error) {
	if len(opcode.data) > 0 {
		num, err := makeScriptNum(opcode.data, true, 5)
		if err != nil {
			return 0, err
		}
		return int64(num), nil
	} else {
		return int64(opcode.opcode.value & 0xF), nil
	}
}

type FileChainParams struct {
	opcode parsedOpcode
	Code   uint8
	Params []interface{}
}

func (p *FileChainParams) String() string {
	params := ""
	for i := 0; i < len(p.Params); i++ {
		if FCEOpCodeDefine[p.Code].ParamFormats[i] == FCEP_DATA_ARRAY {
			params = params + fmt.Sprintf("%d:", i) + hex.EncodeToString(p.Params[i].([]byte)) + " "
		} else {
			params = params + fmt.Sprintf("%d:", i) + fmt.Sprintf("%d", p.Params[i].(int64)) + " "
		}
	}
	return fmt.Sprintf("Code: %s Params: %s", p.opcode.opcode.name, params)
}

func ExtractFCEOperation(script []byte) (*FileChainParams, error) {
	opcodes, err := parseScript(script)
	if err != nil {
		return nil, err
	}
	params := make([]interface{}, 0)
	for idx, code := range opcodes {
		if opdefine, ok := FCEOpCodeDefine[code.opcode.value]; ok {
			if idx >= len(opdefine.ParamFormats) {
				for i := 0; i < len(opdefine.ParamFormats); i++ {
					opcode := opcodes[idx-len(opdefine.ParamFormats)+i]
					if opdefine.ParamFormats[i] == FCEP_NUMBER {
						num, err := extractScriptInt64(opcode)
						if err != nil {
							return nil, err
						} else {
						...
						params = append(params, opcode.data)
					}
				}
				return &FileChainParams{opcode: code, Code: code.opcode.value, Params: params}, nil
			}
		}
	}
	return nil, nil
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []btcutil.Address, int, *FileChainParams, error) {
	var addrs []btcutil.Address
	var requiredSigs int
	var fcparam *FileChainParams

	// No valid addresses or required signatures if the script doesn't
	// parse.
	pops, err := parseScript(pkScript)
	if err != nil {
		return NonStandardTy, nil, 0, fcparam, err
	}

	if len(pops) > 0 {
		if opdefine, ok := FCEOpCodeDefine[pops[len(pops)-1].opcode.value]; ok {
			fcparam, err = ExtractFCEOperation(pkScript)
			if err != nil {
				return NonStandardTy, nil, 0, fcparam, err
			}
			pops = pops[:len(pops)-len(opdefine.ParamFormats)-1]
		}
	}
	...
	return scriptClass, addrs, requiredSigs, fcparam, nil
}
...
func BuyHashTransDepositScript(filehash []byte, buyhash []byte) ([]byte, error) {
	return NewScriptBuilder().addData(buyhash).addData(filehash).AddOp(OP_FILE_BUY_TRANS).Script()
}

func ConfirmHashTransferFeeScript(buyhash []byte, transfer btcutil.Address) ([]byte, error) {
	paybytes, err := PayToAddrScript(transfer)
	if err != nil {
		return nil, err
	}
	builder := NewScriptBuilder().addData(buyhash).AddOp(OP_FILE_CONFIRM_TRANSFEE)
	buybytes, err := builder.Script()
	if err != nil {
		return nil, err
	}

	return append(paybytes, buybytes...), nil
}

func ConfirmBuyFeeScript(buyhash []byte, filehash []byte, author btcutil.Address) ([]byte, error) {
	paybytes, err := PayToAddrScript(author)
	if err != nil {
		return nil, err
	}
	builder := NewScriptBuilder().addData(buyhash).addData(filehash).AddOp(OP_FILE_CONFIRM_BUY)
	buybytes, err := builder.Script()
	if err != nil {
		return nil, err
	}
...
func BuyHashFeeScript(filehash []byte, author btcutil.Address, buyhash []byte) ([]byte, error) {
	paybytes, err := PayToAddrScript(author)
	if err != nil {
		return nil, err
	}
	builder := NewScriptBuilder().addData(buyhash).addData(filehash).AddOp(OP_FILE_BUY_PAY)
	buybytes, err := builder.Script()
	if err != nil {
		return nil, err
	}
...
func PublishScript(hash []byte, price btcutil.Amount, title []byte, author []byte, tag uint8, depositAddr btcutil.Address) ([]byte, error) {
	paybytes, err := PayToAddrScript(depositAddr)
	if err != nil {
		return nil, err
	}
	builder := NewScriptBuilder().addData(hash).AddInt64(int64(price)).addData(title).addData(author).AddInt64(int64(tag)).AddOp(OP_FILE_PUBLISH)
	pubbytes, err := builder.Script()
	if err != nil {
		return nil, err
	}
	return append(paybytes, pubbytes...), nil
}
