...
For example (taken from [f3ec130](https://github.com/filechain/filechain/commit/f3ec13030e4e828869954472cbc51ac36bee5c1d)):
...
1. `git clone https://github.com/filechain/filechain.git`
2. `cd btcd`
...
which roughly follow the format of [previous release notes](https://github.com/filechain/filechain/releases/tag/v0.20.1-beta).
...
5. Ensure `go` is installed, matching the same version as noted in the release
   notes.
6. Obtain a copy of `btcd`'s source code with `git clone https://github.com/filechain/filechain` and checkout the source code of the
...
8. Extract the archive found in the `btcd-<TAG>` directory created by the
   ...
   and btcctl) with `shasum -a 256 <filename>`. These should match **exactly**
