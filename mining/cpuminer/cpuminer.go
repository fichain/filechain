...
func (m *CPUMiner) solveBlock(msgBlock *wire.MsgBlock, blockHeight int32,
	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			hashbig := blockchain.HashToBig(&hash)
			// log.Infof("Cmp diff %X vs %X", hashbig, targetDifficulty)
			if hashbig.Cmp(targetDifficulty) <= 0 {
				m.updateHashes <- hashesCompleted
				return true
			}
		}
...
func (m *CPUMiner) Start() {
	...
	if len(m.cfg.MiningAddrs) > 0 {
		for _, addr := range m.cfg.MiningAddrs {
			log.Info("address:", addr.EncodeAddress())
		}
	}
...
func (m *CPUMiner) SetMiningAddrs(addrs []string) error {
	m.cfg.MiningAddrs = []btcutil.Address{}
	for _, addr := range addrs {
		a, err := btcutil.DecodeAddress(addr, m.cfg.ChainParams)
		if err != nil {
			return err
		}
		m.cfg.MiningAddrs = append(m.cfg.MiningAddrs, a)
	}
	return nil
}
