...
func (b *BlockChain) initChainState() error {
	...
	if !initialized {
		...
		// 初始化文件链数据库，比特币的设计不存在世界状态，因此可以通过 合适的构造 mempool 让所有的内容存储在内存中就完成校验
		return FileChainInitStore(b, true)
	}
	// 初始化文件链数据库，比特币的设计不存在世界状态，因此可以通过 合适的构造 mempool 让所有的内容存储在内存中就完成校验
	// 加入文件链以后，不可能将全链所有的文件信息都存储在内存，也没有这个必要，因此文件链模块记录了一个数据库的索引，以便随时
	// 可以从数据库中查询文件状态
	FileChainInitStore(b, false)
