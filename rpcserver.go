...
var rpcHandlersBeforeInit = map[string]commandHandler{
	...
	"listfilehash":           handleListHash,
	"getfilehashinfo":        handleListSingleHash,
	"getbuyhashinfo":         handleGetBuyHashInfo,
}
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		// it anyways.
		scriptClass, addrs, reqSigs, _, _ := txscript.ExtractPkScriptAddrs(
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	scriptClass, addrs, reqSigs, _, _ := txscript.ExtractPkScriptAddrs(script,
		s.cfg.ChainParams)
...
func handleGetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	return &struct {
		...
		Addresses []string
		HashRate  float64
	}{Mining: s.cfg.CPUMiner.IsMining(), Addresses: cfg.MiningAddrs, HashRate: s.cfg.CPUMiner.HashesPerSecond()}, nil
}
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	scriptClass, addrs, reqSigs, _, _ := txscript.ExtractPkScriptAddrs(pkScript,
		s.cfg.ChainParams)
	addresses := make([]string, len(addrs))
	for i, addr := range addrs {
		addresses[i] = addr.EncodeAddress()
	...
	if !generate {
		...
		if len(cfg.miningAddrs) == 0 {
			if len(c.MinerAddr) == 0 {
				...
				cfg.MiningAddrs = append(cfg.MiningAddrs, c.MinerAddr)
				addr, err := btcutil.DecodeAddress(c.MinerAddr, s.cfg.ChainParams)
				if err != nil {
					return nil, err
				}
				cfg.miningAddrs = append(cfg.miningAddrs, addr)
				s.cfg.CPUMiner.SetMiningAddrs(cfg.MiningAddrs[:])
			}
		}
...
// 询问在线hash的情况
func handleListHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.ListHashCmd)
	return blockchain.ListHash(c)
}

//查询单个hash的详情
func handleListSingleHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.ListSingleHashCmd)
	return blockchain.ListSingleHash(c)
}

func handleGetBuyHashInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.GetBuyHashCmd)
	return blockchain.GetBuyHashInfo(c)
}
