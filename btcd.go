...
func serverHttp(port int) {
	err := http.ListenAndServe(":"+strconv.Itoa(18000), nil)
	if err != nil {
		btcdLog.Errorf("Error on http", err)
	} else {
		btcdLog.Infof("Listening on :%d...", port)
	}
}
...
func btcdMain(serverChan chan<- *server) error {
	...
	htmlFS, _ := fs.Sub(staticFiles, "static")
	...
	http.HandleFunc("/cfg", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "window.fccfg="+string(json))
	})
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		f, err := htmlFS.Open(upath[1:])
		if err != nil && !strings.HasSuffix(upath, ".map") {
			r.URL.Path = "/"
			fs.ServeHTTP(w, r)
		} else {
			if f != nil {
...
func StartWalletMain(cfg *config) {

	// 将 cfg 里的配置转移到 walletConfig
	walletConfig := &utils.Config{
		ConfigFile:               nil,
		ShowVersion:              false,
		Create:                   false,
		CreateTemp:               false,
		AppDataDir:               utils.NewExplicitString(cfg.DataDir),
		TestNet3:                 cfg.TestNet3,
		SimNet:                   cfg.SimNet,
		SigNet:                   cfg.SigNet,
		SigNetChallenge:          cfg.SigNetChallenge,
		SigNetSeedNode:           cfg.SigNetSeedNode,
		NoInitialLoad:            true,
		...
		DisableClientTLS:         cfg.DisableTLS,
		BtcdUsername:             cfg.RPCUser,
		BtcdPassword:             cfg.RPCPass,
		Proxy:                    cfg.Proxy,
		...
		RPCCert:                  utils.NewExplicitString(""),
		...
		DataDir:                  utils.NewExplicitString(cfg.DataDir),
	}
	if err := utils.WalletMain(walletConfig); err != nil {
		os.Exit(1)
	}
}
